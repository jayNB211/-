...
public class SendEmailMapperTest {
	...
	*/
	...
	@Test
	public void testQueryAllSendEmailsByPaging() {

		try {
			PagingVO pagingVo = new PagingVO();
			pagingVo.setTotalCountPage(1);
			pagingVo.setToPage(1);
			List<SendEmail> listSendEmail = mapper.queryAllSendEmailsByPaging(pagingVo);
			Iterator<SendEmail> its = listSendEmail.iterator();
			while (its.hasNext()) {
	...
	@Test
	public void testCountSendEmails() {
		try {
			int i = this.mapper.countSendEmails();
			if (i > 0) {
				log.info(i);
				session.commit();
			}
