...
public class ParameterServiceImplTest extends SpringTestBase{
	...
	public void testAddParameter() {
		...
		patameter.setParameterCreatedName("admin");
		patameter.setParameterCreatedDatetime(new Timestamp(new Date().getTime()));
		patameter.setParameterUpdatedName("admin");
	...
	public void testfindByName() {
		try{
			...
			List<Parameter> pat=new ArrayList<>();
			pat = this.patService.findByName("fi");
			Iterator<Parameter> its=pat.iterator();
			while(its.hasNext()){
				System.out.println(its.next());
			}
